...
export function SliderServiceSection({
  content,
}: {
  content: SliderServiceSectionType[];
}) {
  const [value, setValue] = React.useState<string>(content?.[0]?.title);

  const activeContent =
    content?.find((c) => c?.title === value) || content?.[0];

  return (
    <section className={cn("my- md:my-0")} data-debug={"home.services-section"}>
