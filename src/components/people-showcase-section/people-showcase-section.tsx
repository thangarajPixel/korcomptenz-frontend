...
import AdvisoryPeopleShowcaseCard from "./_utils/advis-people-showcase-card";
  ...
  return (
    <section data-debug="people-showcase-section" id="our-leaders">
      <div className="flex flex-col gap-10">
        ...
        <div
          className={cn(
            "container-md grid  grid-cols-1 md:grid-cols-2 gap-6 md:gap-8",

            list?.isAdvisoryBoard ? "lg:grid-cols-2" : "lg:grid-cols-3",
          )}
        >
          {list?.list.map((data, index) =>
            list?.isAdvisoryBoard ? (
              <AdvisoryPeopleShowcaseCard key={index} data={data} />
            ) : (
