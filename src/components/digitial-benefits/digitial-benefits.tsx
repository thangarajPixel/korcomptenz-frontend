...
const DigitialBenefits = ({ data }: { data: DigitalBenefitsType }) => {
  return (
    <section
      className={cn(
        data?.isbgGrey ? "bg-white " : "bg-[#E8F1EC] md:p-8 md:p-12 p-4",
      )}
    >
      <div className={cn("container-md")}>
        <div
          className={cn(
            data?.isbgGrey
        ...
        >
          ...
          >
            {data?.list?.map((badge) => (
              <div key={badge?.id} className="flex flex-col  gap-2  md:px-4">
                {/* Icon */}
                ...
                <DangerousHtml
                  ...
                  className="text-lg leading-relaxed text-foreground text-left"
                />
              </div>
            ))}
          </div>
