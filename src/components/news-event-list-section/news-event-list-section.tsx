...
export const groupByYear = (data: NewsEventListSectionType[]) => {
  const groups: Record<string, { date: string; item: NewsEventListSectionType[] }> = {};
...
const NewsEventListSectionItem = ({ data: item }: { data: NewsEventListSectionType }) => {
  return (
...
export default function NewsEventListSection({ data }: { data: NewsEventListSectionType[] }) {
  ...
  return (
    <div className="relative flex flex-col lg:flex-row bg-white container-md">
      ...
      <div ref={contentRef} className="flex-1 overflow-y-auto pl-4">
        {/* Content Sections */}
        {
          data?.[0]?.isEvent ? groupByYear(data)?.map((section, index) => (
            <div
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {section?.item?.map((item) => (
                  <NewsEventListSectionItem key={`section-item-${item.id}`} data={item} />
                ))}
              </div>
            </div>
          ))
