...
export default function SearchChips({
  ...
  const { data } = useCaseStudySearchHook({ search: "sap" });
  const [chips, setChips] = React.useState<Chip[]>(data);
  // console.log(data);

  const toggle = React.useCallback(
    (id: string) => {
      setChips((prev) => {
        const updatedChips = prev.map((c) =>
          c.id === id ? { ...c, selected: !c.selected } : c
        );
        onChange(
          updatedChips.reduce((acc, chip) => {
            if (chip.selected) {
              acc[chip.from] = [chip.id];
            }
            return acc;
  ...
  const selected = chips?.filter((c) => c.from);
